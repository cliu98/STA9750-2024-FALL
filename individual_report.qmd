...
The next set of data are coordinate systems for each country in the world. These are readily available from [Google Developers](https://developers.google.com/public-data/docs/canonical/countries_csv). This data can be web scraped and imported into the work environment directly.
...
climate_interaction <- medal_tally |>
  select(year, country, total, edition) |>
  left_join(climate_zones, by = "country") |>
  mutate(edition = case_when( # rename for Shiny UI
    grepl("Summer", edition, ignore.case = TRUE) ~ "Summer",
    grepl("Winter", edition, ignore.case = TRUE) ~ "Winter",
    TRUE ~ edition
  ))

# define UI
ui <- fluidPage(
  titlePanel("Interactive Olympic Results by Climate Region"),
  
  sidebarLayout(
    sidebarPanel(
      sliderInput("yearRange", "Select Year Range:",
                  min = min(climate_interaction$year),
                  max = max(climate_interaction$year),
                  value = c(min(climate_interaction$year), max(climate_interaction$year)),
                  step = 1),
      pickerInput("selectedRegions", "Select Climate Regions:", 
                  choices = na.omit(unique(climate_interaction$region)),  # Remove NA
                  selected = unique(climate_interaction$region),
                  multiple = TRUE,
                  options = list(`actions-box` = TRUE)),
      pickerInput("selectedEditions", "Select Editions:", 
                  choices = unique(climate_interaction$edition),
                  selected = unique(climate_interaction$edition),
                  multiple = TRUE,
                  options = list(`actions-box` = TRUE))
    ),
  ...
  filteredData <- reactive({
    climate_interaction |>
      filter(year >= input$yearRange[1],
             ...
             edition %in% input$selectedEditions)
  })
  
  output$barPlot <- renderPlot({
    ggplot(filteredData(), aes(x = region, y = total, fill = region)) +
      geom_bar(stat = "identity", position = "dodge") +
      labs(title = "Total Medals by Climate Region",
           x = "Climate Region",
           y = "Total Medals") +
      theme_minimal()
  })
